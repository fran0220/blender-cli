...
| Run-1 item: `progress` per the feedback policy, and a `patience` stop reported in `done.stopped` | done on Linux — under the default `improvements` a 40-evaluation fit emits exactly the 9 events of its curve where it used to emit 40; `all` stays a 0.5 s heartbeat and `off` emits none. On L's scene shape (5 program parameters, 60 evaluations) the default patience 16 ends the search at 24 of 60, 66.2 s of 162.5 s (−59 %), but at IoU 0.959 against 0.997 — a cyclic cycle over 5 parameters is 10 barren evaluations, so 16 stops one halving too early. Patience 24 and 32 both run the full budget and lose nothing. The default belongs to K's contract table; the measurement and the `4n` guidance are in design.md, and raising the default is filed to K |
